...
const func = async ({ plasmic }: HardhatRuntimeEnvironment) => {
  const { getTokens } = plasmic;
  ...
  for (const [tokenSymbol, token] of Object.entries(tokens)) {
    ...
    if (token.dataStreamSpreadReductionFactor) {
      await setUintIfDifferent(
        keys.dataStreamSpreadReductionKey(token.address),
        token.dataStreamSpreadReductionFactor,
        `data stream spread reduction factor for ${tokenSymbol} ${token.address}`
      );
    }
  }
};

func.tags = ["DataStreamFeeds"];
