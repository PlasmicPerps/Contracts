...
async function main() {
  const oracleConfig = await hre.plasmic.getOracle();
  ...
  if (oracleConfig.pythFeedSpreadReductionBps) {
    for (const [feedId, reductionBps] of Object.entries(oracleConfig.pythFeedSpreadReductionBps)) {
      const newValue = reductionBps;
      const oldValue = await pythDataStreamProvider.spreadReductionBps(feedId);
      if (newValue.toString() !== oldValue.toString()) {
        console.log("updated spread reduction for %s: %s -> %s", feedId, oldValue, newValue);

        if (process.env.WRITE === "true") {
          console.log("sending transaction...");
          const tx = await pythDataStreamProvider.setSpreadReductionBps(feedId, newValue);
          console.log("tx sent: %s", tx.hash);
        }
      }
    }
