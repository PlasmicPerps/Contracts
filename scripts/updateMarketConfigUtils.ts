...
const processMarkets = async ({ markets, onchainMarketsByTokens, tokens, generalConfig, handleConfig }) => {
  for (const marketConfig of markets) {
    ...
    if (marketConfig.withdrawalFeeFactor) {
      await handleConfig(
        "uint",
        keys.WITHDRAWAL_FEE_FACTOR,
        encodeData(["address"], [marketToken]),
        marketConfig.withdrawalFeeFactor,
        `withdrawalFeeFactor ${marketLabel} (${marketToken})`
      );
    }
...
export async function updateMarketConfig({ write }) {
  if (
    !["arbitrumGoerli", "avalancheFuji", "zkSync", "zkSyncInternal", "sonic", "sonicInternal", "hardhat"].includes(
      network.name
    )
  ) {
  ...
  const generalConfig = await hre.plasmic.getGeneral();
  const tokens = await hre.plasmic.getTokens();
  const markets = await hre.plasmic.getMarkets();
