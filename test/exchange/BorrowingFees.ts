...
import { getClaimableInsuranceAmount } from "../../utils/fee";
...
describe("Exchange.BorrowingFees", () => {
  ...
  it("borrowing fees", async () => {
    ...
    await dataStore.setUint(keys.POSITION_INSURANCE_FUND_FACTOR, decimalToFloat(1, 1)); // 10%
    ...
    await usingResult(
      getMarketTokenPriceWithPoolValue(fixture, { prices: prices.ethUsdMarket }),
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).closeTo("1000961638898586187144864970680", "10000000000000000000000"); // 1.00106848761
        expect(poolValueInfo.poolValue).closeTo(
          "6005769833391517122869189824084872000",
    ...
    expect(await getClaimableInsuranceAmount(dataStore, ethUsdMarket.marketToken, wnt.address)).eq("19353841103099300"); // 0.0014989s

    await usingResult(
      getMarketTokenPriceWithPoolValue(fixture, { prices: prices.ethUsdMarket }),
      ([marketTokenPrice, poolValueInfo]) => {
        expect(marketTokenPrice).closeTo("1000961643292439911420000000000", "10000000000000000000000"); // 1.00106849254
        expect(poolValueInfo.poolValue).closeTo(
          "6005769833391517122869189824084872000",
          "100000000000000000000000000000"
        ); // 6006410.95527
