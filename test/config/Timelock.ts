...
describe("Timelock", () => {
  ...
  it("setInsuranceFund", async () => {
    await expect(timelock.connect(user2).signalSetInsuranceFund(user3.address))
      .to.be.revertedWithCustomError(errorsContract, "Unauthorized")
      .withArgs(user2.address, "TIMELOCK_ADMIN");

    await timelock.connect(timelockAdmin).signalSetInsuranceFund(user3.address);

    await expect(timelock.connect(user2).setInsuranceFundAfterSignal(user3.address))
      .to.be.revertedWithCustomError(errorsContract, "Unauthorized")
      .withArgs(user2.address, "TIMELOCK_ADMIN");

    await expect(
      timelock.connect(timelockAdmin).setInsuranceFundAfterSignal(user3.address)
    ).to.be.revertedWithCustomError(errorsContract, "SignalTimeNotYetPassed");

    await time.increase(1 * 24 * 60 * 60 + 10);

    expect(await dataStore.getAddress(keys.INSURANCE_FUND)).eq(ethers.constants.AddressZero);

    await timelock.connect(timelockAdmin).setInsuranceFundAfterSignal(user3.address);

    expect(await dataStore.getAddress(keys.INSURANCE_FUND)).eq(user3.address);
  });
