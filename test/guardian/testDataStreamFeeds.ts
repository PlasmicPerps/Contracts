...
describe("Guardian.DataStreamFeeds", () => {
  ...
  let fixture;
  let user1;
  let reader, dataStore, mockPyth, solUsdMarket, solAddr, ethUsdMarket, wnt, usdc;

  beforeEach(async () => {
    fixture = await deployFixture();

    ({ user1 } = fixture.accounts);
    ({ reader, dataStore, mockPyth, solUsdMarket, ethUsdMarket, wnt, usdc } = fixture.contracts);
  ...
  it("Order executes with data stream feed tokens only", async () => {
    ...
    await dataStore.setUint(keys.dataStreamMultiplierKey(wnt.address), expandDecimals(1, 42));
    await dataStore.setBytes32(keys.dataStreamIdKey(usdc.address), hashString("USDC"));
    await dataStore.setUint(keys.dataStreamMultiplierKey(usdc.address), expandDecimals(1, 54));
    ...
    let updateDataWnt = await mockPyth.createPriceFeedUpdateData(
      hashString("WNT"),
      expandDecimals(5001, 8),
      expandDecimals(1, 8),
      -8,
      expandDecimals(5001, 8),
      expandDecimals(1, 8),
    ...
    let updateDataUSDC = await mockPyth.createPriceFeedUpdateData(
      hashString("USDC"),
      expandDecimals(1, 8),
      0,
      -8,
    ...
    await executeOrder(fixture, {
      ...
      dataStreamData: [updateDataWnt, updateDataUSDC],
    ...
    updateDataWnt = await mockPyth.createPriceFeedUpdateData(
      hashString("WNT"),
      expandDecimals(5000, 8),
      0,
      -8,
      expandDecimals(5000, 8),
      0,
    ...
    updateDataUSDC = await mockPyth.createPriceFeedUpdateData(
      hashString("USDC"),
      expandDecimals(1, 8),
      0,
      -8,
  ...
  // only using pyth data streams for now
