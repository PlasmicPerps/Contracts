...
describe("Guardian.Fees", () => {
  ...
  it("FeeKeeper claims fees through FeeHandler", async () => {
    ...
    await dataStore.setUint(keys.POSITION_INSURANCE_FUND_FACTOR, decimalToFloat(2, 1)); // 20%
    ...
    const claimableInsuranceFeeAmount = await dataStore.getUint(
      keys.claimableInsuranceFeeAmountKey(ethUsdMarket.marketToken, usdc.address)
    );
    expect(claimableInsuranceFeeAmount).to.eq(0);
    ...
    // 20% * $250 = $50 for the insuranceFund
    ...
    // 20% * $125 = $25 for the insuranceFund
    const positionFeeFromDecrease = expandDecimals(125, 5);
    const insuranceFeeFromDecrease = expandDecimals(250, 5);
    ...
    await dataStore.setAddress(keys.INSURANCE_FUND, wallet.address);
    ...
    expect(keeperBalAfter.sub(keeperBalBefore)).to.eq(claimableFeeAmount.add(claimableInsuranceFeeAmount));
