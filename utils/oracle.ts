...
export async function getOracleParams({
  ...
  const pythDataStreamProvider = await hre.ethers.getContract("PythDataStreamProvider");
  const storkPriceFeedProvider = await hre.ethers.getContract("StorkPriceFeedProvider");
  ...
  for (let i = 0; i < dataStreamTokens.length; i++) {
    ...
    const providerAddress =
      token.oracleProvider === "storkOracle" ? storkPriceFeedProvider.address : pythDataStreamProvider.address;
    await dataStore.setAddress(keys.oracleProviderForTokenKey(token), providerAddress);
    params.tokens.push(token);
    params.providers.push(providerAddress);
