...
export function createDeployFunction({
  ...
  const func = async ({ getNamedAccounts, deployments, plasmic, network }: HardhatRuntimeEnvironment) => {
    ...
    let deployArgs = [];
    if (getDeployArgs) {
      deployArgs = await getDeployArgs({ dependencyContracts, network, plasmic, get, getNamedAccounts });
    }
    ...
    if (afterDeploy) {
      await afterDeploy({ deployedContract, deployer, getNamedAccounts, deployments, plasmic, network });
    }
