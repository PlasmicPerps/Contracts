...
library Keys {
    ...
    // @dev for sending received fees to insurance
    bytes32 public constant INSURANCE_FUND = keccak256(abi.encode("INSURANCE_FUND"));
    ...
    // @dev key for the claimable fee amount
    bytes32 public constant CLAIMABLE_INSURANCE_FEE_AMOUNT = keccak256(abi.encode("CLAIMABLE_INSURANCE_FEE_AMOUNT"));
    ...
    // @dev key for the percentage amount sent to insurance fund
    bytes32 public constant POSITION_INSURANCE_FUND_FACTOR = keccak256(abi.encode("POSITION_INSURANCE_FUND_FACTOR"));
    ...
    bytes32 public constant DATA_STREAM_ID = keccak256(abi.encode("DATA_STREAM_ID"));
     // @dev key for data stream base feed id
    bytes32 public constant DATA_STREAM_BASE_ID = keccak256(abi.encode("DATA_STREAM_BASE_ID"));
    ...
    // @dev key for data stream spread reduction factor
    ...
    // @dev key for the claimable insurance fee amount
    // @param market the market for the fee
    // @param token the token for the fee
    function claimableInsuranceFeeAmountKey(address market, address token) internal pure returns (bytes32) {
        return keccak256(abi.encode(CLAIMABLE_INSURANCE_FEE_AMOUNT, market, token));
    }
   ...
   function virtualInventoryForSwapsKey(bytes32 virtualMarketId, bool isLongToken) internal pure returns (bytes32) {
    ...
    function withdrawalFeeFactorKey(address market) internal pure returns (bytes32) {
        return keccak256(abi.encode(
    ...
    // @dev key for data stream base feed ID
    // @param token the token to get the key for
    // @return key for data stream base feed ID
    function dataStreamBaseIdKey(address token) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            DATA_STREAM_BASE_ID,
            token
        ));
    ...
    // @dev key for data stream spread reduction factor
    // @param token the token to get the key for
    // @return key for data stream spread reduction factor
