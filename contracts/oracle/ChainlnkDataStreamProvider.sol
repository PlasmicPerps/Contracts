...
contract ChainlinkDataStreamProvider is IOracleProvider {
    ...
    /// @dev if Chainlink Provider call without sending msg.value
    function isChainlinkProvider() external pure returns (bool) {
        return true;
    }

    function getOraclePriceFee(address /* tokens */, bytes memory /* data */) external view returns (uint256) {
        return 0;
    }
    ...
    function getOraclePrice(
        address token,
        bytes memory data
    ) external payable onlyOracle returns (OracleUtils.ValidatedPrice memory) {
        bytes32 feedId = dataStore.getBytes32(Keys.dataStreamIdKey(token));
        if (feedId == bytes32(0)) {
            revert Errors.EmptyDataStreamFeedId(token);
        }

        bytes memory verifierResponse = verifyReport(data);
     ...
     function verifyReport(bytes memory unverifiedReport) internal returns(bytes memory) {
