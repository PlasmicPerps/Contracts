...
contract Oracle is RoleModule {
    ...
    // TODO: find alternative for pyth
    ...
    function _validatePrices(
        ...
        for (uint256 i; i < params.tokens.length; i++) {
            ...
            bool isChainlinkProvider = IOracleProvider(provider).isChainlinkProvider();
            OracleUtils.ValidatedPrice memory validatedPrice;

            if (isChainlinkProvider) {
                validatedPrice = IOracleProvider(provider).getOraclePrice(token, data);
            } else {
                uint256 fee = IOracleProvider(provider).getOraclePriceFee(token, data);
                validatedPrice = IOracleProvider(provider).getOraclePrice{value: fee}(token, data);
            }
