...
library ExecuteWithdrawalUtils {
    ...
    struct _ExecuteWithdrawalCache {
        ...
        uint256 longTokenWithdrawalFees;
        uint256 shortTokenWithdrawalFees;
    ...
    function _executeWithdrawal(
        ...
        cache.longTokenWithdrawalFees = _getWithdrawalFee(
            params.dataStore,
            market.marketToken,
            cache.longTokenOutputAmount
        );
        ...
        FeeUtils.incrementClaimableFeeAmount(
            ...
            market.marketToken,
            market.longToken,
            cache.longTokenFees.feeReceiverAmount + cache.longTokenWithdrawalFees,
        ...
        cache.shortTokenWithdrawalFees = _getWithdrawalFee(
            params.dataStore,
            market.marketToken,
            cache.shortTokenOutputAmount
        );
        ...
        FeeUtils.incrementClaimableFeeAmount(
            ...
            market.marketToken,
            market.shortToken,
            cache.shortTokenFees.feeReceiverAmount + cache.shortTokenWithdrawalFees,
            Keys.WITHDRAWAL_FEE_TYPE
        );
        ...
        cache.longTokenOutputAmount = cache.longTokenFees.amountAfterFees - cache.longTokenWithdrawalFees;

        cache.shortTokenPoolAmountDelta = cache.shortTokenOutputAmount - cache.shortTokenFees.feeAmountForPool;
        cache.shortTokenOutputAmount = cache.shortTokenFees.amountAfterFees - cache.shortTokenWithdrawalFees;
    ...
    function _getWithdrawalFee(DataStore dataStore, address market, uint256 amount) internal view returns (uint256) {
        uint256 withdrawalFeeFactor = dataStore.getUint(Keys.withdrawalFeeFactorKey(market));
        return Precision.applyFactor(amount, withdrawalFeeFactor);
    }
}
