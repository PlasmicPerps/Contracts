...
contract Timelock is ReentrancyGuard, RoleModule, BasicMulticall {
    ...
    // @dev signal setting of the fee receiver
    // @param account the new fee receiver
    function signalSetInsuranceFund(address account) external onlyTimelockAdmin nonReentrant {
        if (account == address(0)) {
            revert Errors.InvalidInsuranceFund(account);
        }

        bytes32 actionKey = _setInsuranceFundActionKey(account);
        _signalPendingAction(actionKey, "setInsuranceFund");

        EventUtils.EventLogData memory eventData;
        eventData.addressItems.initItems(1);
        eventData.addressItems.setItem(0, "account", account);
        eventEmitter.emitEventLog1(
            "SignalSetInsuranceFund",
            actionKey,
            eventData
        );
    }
    ...
    // @dev set the fee receiver
    // @param account the new fee receiver
    function setInsuranceFundAfterSignal(address account) external onlyTimelockAdmin nonReentrant {
        bytes32 actionKey = _setInsuranceFundActionKey(account);
        _validateAndClearAction(actionKey, "setInsuranceFund");

        dataStore.setAddress(Keys.INSURANCE_FUND, account);
    ...
    function _setInsuranceFundActionKey(address account) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked("setInsuranceFund", account));
    }
